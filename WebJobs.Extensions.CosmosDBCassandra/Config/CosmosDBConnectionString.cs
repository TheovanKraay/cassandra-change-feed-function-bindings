...
namespace Microsoft.Azure.WebJobs.Extensions.CosmosDBCassandra.Config
{
    /// <summary>
    ...
    internal class CosmosDBConnectionString
    {
        public CosmosDBConnectionString(string connectionString)
        {
            ...
            if (builder.TryGetValue("ContactPoint", out object contactpoint))
            {
                ContactPoint = contactpoint.ToString();
            }

            if (builder.TryGetValue("User", out object user))
            {
                User = user.ToString();
            }
            if (builder.TryGetValue("Password", out object password))
            {
